...
def update_plan_status(target, module, success):
    ...
    try:
        ...
        if result:
            ...
            # Update step status - check both module and name fields
            for step in plan.get("steps", []):
                if step.get("module") == module or step.get("name") == module:
            ...
            conn.commit()
            logger.info(f"Updated plan status for {target}")
        else:
            logger.error(f"No plan found for {target}")
    except Exception as e:
